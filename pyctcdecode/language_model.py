...
class HotwordScorer:
    def __init__(
        ...
        self._hotword_ngrams = hotword_ngrams
        self._hotwords_weight_map = hotwords_weight_map
        self.regex_str = re.compile('|'.join(hotword_ngrams))
        self.hot_maxlen = max(map(len, hotword_ngrams)) + 18
        chars = list(set(''.join(hotword_ngrams)))
        self.chars = {}
        for c in chars:
            self.chars[c] = True
    ...
    def __contains__(self, item: str) -> bool:
        ...
        for gram in self._hotword_ngrams:
            if gram in item:
                return True
    ...
    def score_text(self, text: str) -> float:
        """Contains."""
        weight_sum = 0
        if len(text) > self.hot_maxlen:
            text = text[-self.hot_maxlen:]

        ret = self.regex_str.findall(text)
        for gram in ret:
            weight_sum += self._hotwords_weight_map[gram]
        ...
        if len(hotwords) > 0:
            ...
            match_ptn = re.compile(
                r"|".join(
                    [
                        r"(?<!\S)" + re.escape(s) + r"(?!\S)"
                        for s in sorted(hotword_ngrams, key=len, reverse=True)
                    ]
                )
            )

            # create trie for partial word matches
            char_trie = CharTrie.fromkeys(hotword_ngrams)
        else:
            # make an unmatchable pattern
            match_ptn = re.compile(r"^\b$")
            # empty trie
            char_trie = CharTrie()
        return cls(match_ptn, char_trie, hotword_ngrams, hotwords_weight_map)
