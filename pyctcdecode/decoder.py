...
class BeamSearchDecoderCTC:
    ...
    def _get_lm_beams(
        ...
        cached_hotword_scores: Dict[str, float],
        is_eos: bool = False,
    ) -> List[LMBeam]:
        ...
        for text, next_word, word_part, last_char, frame_list, frames, logit_score in beams:
            ...
            if new_text not in cached_lm_scores:
                ...
                cached_lm_scores[new_text] = (raw_lm_score, raw_lm_score, end_state)
            ...
            text_part = _merge_tokens(new_text, word_part)
            if word_part not in hotword_scorer.chars:
                if new_text not in cached_hotword_scores:
                    cached_hotword_scores[new_text] = cached_hotword_scores[text_part] = hotword_scorer.score_text(new_text)
                cached_hotword_scores[text_part] = cached_hotword_scores[new_text]
                lm_score += cached_hotword_scores[text_part]
            else:
                hotword_score = hotword_scorer.score_text(text_part)
                cached_hotword_scores[text_part] = hotword_score
                lm_score += hotword_score
